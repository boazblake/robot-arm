...
const TrackingViewer: m.Component = {
  ...
  view: () => {
    ...
    return m("section.tracking-viewer", [
      ...
      m("div.tracking-toolbar", [m("strong", "Human-motion tracking"), m("span", `Pose ${frame.poseLandmarks.length} · Hands ${frame.leftHandLandmarks.length || frame.rightHandLandmarks.length}`), m("ion-button", { size: "small", onclick: () => void start() }, state() === "Streaming" ? "Tracking" : "Start")]),
